...
class CommandDispatcher:
    ...
    def resolve_action(cls, command):
        ...
        # LANCER
        if action == "LANCER":
            if (subject == "ENQUETE" and arguments.get("nom") == "CYBERSECURITE"):
                return "ENQUETE_CYBERSECURITE"

            if (subject == "CAMPAGNE" and arguments.get("nom")== "ECOLE"):
                return "CAMPAGNE_ECOLE"

        # CHERCHER
        if action == "CHERCHER":
            tokens = arguments.get("tokens", [])
            if (subject == "ENFANTS" and tokens == ["KINSHASA"]):
                return

            if (subject == "ADOLESCENTS" and tokens ==
                [
                    "INTERESSES",
                    "PAR",
                    "PYTHON"
    ...
    def dispatch(cls, command, data=None):
        ...
        # RESOLUTION
        action = cls.resolve_action(command)
        ...
        # HANDLER
        handler_class = cls.HANDLERS.get(action)
        ...
        return handler.handle(handler_command, data=data)