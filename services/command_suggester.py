...
class CommandSuggester:
    ...
    def similarity(word, candidate):
        ...
        return SequenceMatcher(None, str(word).upper(), str(candidate).upper()).ratio()

    # SUGGESTION UNIQUE
    @classmethod
    def suggest(cls, word, limit=3):
        if not word:
            return []

        word = str(word).upper()
        mots = list(RESERVED_WORDS.keys())
        candidates = get_close_matches(word, mots, n=limit, cutoff=CorrectionConfig.SUGGEST_THRESHOLD)
        suggestions = []

        for candidate in candidates:
            score = cls.similarity(word, candidate)
            suggestions.append(
                {
                    "word": candidate,
                    "score": score
    ...
    @classmethod
    def suggest_multiple(cls, word, limit=3):
        ...
        word = str(word).upper()
        mots = list(RESERVED_WORDS.keys())
        suggestions = get_close_matches(word, mots, n=limit, cutoff=CorrectionConfig.SUGGEST_THRESHOLD)
        return suggestions
    ...
    def decide(cls, word
        ...
        if word in RESERVED_WORDS:
            return {
                ...
                "suggestion": word,
                "score": 1.0,
                "trace": [f"{word} est un mot réservé valide."]
            }

        # RECHERCHE DU MEILLEUR CANDIDAT
        mots = list(RESERVED_WORDS.keys())
        candidates = get_close_matches(word, mots, n=1, cutoff=CorrectionConfig.SUGGEST_THRESHOLD)
        ...
        if (score >= CorrectionConfig.AUTO_CORRECT_THRESHOLD):
            trace.append("Décision : AUTO_CORRECT")
        ...
        # REFORMULATE
        trace.append("Décision : REFORMULATE")
    ...
    def analyze(cls, word):
        ...
        """
        decision = cls.decide(word)
        suggestions = cls.suggest_multiple(word)
    ...
    def display_trace(cls, result):
        ...
        print("=" * 70)
        print(f"Mot : " f"{result.get('word')}")
        print(f"Mode : " f"{result.get('mode')}")
        print(f"Score : " f"{result.get('score', 0.0):.4f}")
        print(f"Suggestion : " f"{result.get('suggestion')}")
        print()
