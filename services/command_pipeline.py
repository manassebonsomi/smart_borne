...
class CommandPipeline:
    ...
    def process(cls, command):
        ...
        if mode == "VALID":
            ...
            return {
                "success": True,
                "mode": "VALID",
                "original": correction.get(
                    "original"
                ),
                "command": correction.get(
                    "corrected"
                ),
                "suggestion": None,
                "score": correction.get(
                    "score",
                    1.0
                ),
                "suggestions": correction.get(
                    "suggestions",
                    []
                ),
                "trace": correction.get(
                    "trace",
                    []
                )
            }
    ...
    def execute(
        ...
        Les modes SUGGEST et REFORMULATE
        s'arrêtent avant le lexer.
        """
        ...
        command_to_parse = correction.get(
            "command"
        )
    ...
    def can_continue(result):
        """
        Détermine si le pipeline peut poursuivre
        l'exécution de la commande.

        Les modes VALID et AUTO_CORRECT permettent
        de continuer automatiquement.

        Les modes SUGGEST, REFORMULATE et les erreurs
        nécessitent une intervention ou une nouvelle saisie.
        """
