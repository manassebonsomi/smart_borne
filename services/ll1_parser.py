from services.grammar import (GRAMMAR, START_SYMBOL, EOF, EPSILON)
...
class LL1Parser:
    ...
    # PARSE
    @classmethod
    def parse(cls, tokens, return_trace=False):
        trace = []
        errors = []
        ...
        if not validation["success"]:
            result = {
                ...
                "message": "La grammaire contient un conflit LL(1).",
                "errors": [],
                "trace": trace
            }
        ...
        if token_list[-1].type != EOF:
            token_list.append(type("EOFToken", (), {"type": EOF, "value": EOF})())
        ...
        while stack:
            if index >= len(token_list):
                error = {
                    "type": "UNEXPECTED_EOF",
                    "message": "Fin inattendue des tokens."
                }
            ...
            trace_step = {
                "step": len(trace) + 1,
                "stack": list(reversed(stack)) + [top],
                "input":[token.type for token in token_list[index:]],
                "lookahead": lookahead_type,
                "action": None
            }

            # ACCEPTATION
            if (top == EOF and lookahead_type == EOF):
                trace_step["action"] = ("ACCEPT" if not errors else "FIN DE L'ANALYSE AVEC ERREURS")
            ...
            if top not in GRAMMAR:
                ...
                if top == lookahead_type:
                    trace_step["action"] = (f"Correspondance : {top}")
                    trace.append(trace_step)
                    index += 1
                    continue
            ...
            # NON-TERMINAL
            production = table.get(top, {}).get(lookahead_type)

            # PRODUCTION TROUVÉE
            if production is not None:
                trace_step["action"] = (f"{top} → " + " ".join(production))
                trace.append(trace_step)
            ...
            # FOLLOW(A)
            non_terminal_follow = follow.get(top, set())
            ...
            while index < len(token_list):
                current_type = (token_list[index].type)

                # Une production est maintenant disponible.
                if current_type in table.get(top, {}):
                    break
        ...
        if index < len(token_list):
            remaining = [token.type for token in token_list[index:]]
            non_eof_remaining = [token for token in remaining if token != EOF]
            ...
            if non_eof_remaining:
                ...
                trace.append({
                    ...
                    "stack": [],
                    "input": remaining,
                    "lookahead": token_list[index].type,
                    "action":
                        (
                            "ERREUR : entrée "
                            "inattendue après "
    ...
    def display_trace(cls, result):
        ...
        for step in trace:
            print()
            print(f"Étape {step['step']}")
            print("Pile :")
            print("  " + " ".join(step["stack"]))
            print("Entrée restante :")
            print("  " + " ".join(step["input"]))
            print("Lookahead :")
            print("  " + step["lookahead"])
            print("Action :")
        ...
        if errors:
            print("ERREURS DÉTECTÉES : " f"{len(errors)}")

            for index, error in enumerate(errors, start=1):
                print()
                print(f"{index}. " f"{error.get('message')}")

            print()

        # RESULTAT
        if result.get("success"):
            print("RESULTAT : ACCEPTATION")
