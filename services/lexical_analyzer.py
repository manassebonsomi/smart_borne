...
class LexicalAnalyzer:
    ...
    def tokenize(command):
        ...
        words = re.findall(r"\d+|[A-ZÀ-ÖØ-Ý]+",command)
        tokens = []
        ...
        for word in words:
            ...
            # Nombre
            if word.isdigit():
                tokens.append(Token("NUMERO", int(word)))
                continue
            ...
            if suggestion:
                raise Exception(
                    f"Token inconnu : " f"{word}. "
                    f"Voulez-vous dire " f"'{suggestion}' ?"
                )

            raise Exception(f"Token inconnu : {word}")
