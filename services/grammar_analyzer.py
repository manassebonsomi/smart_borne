from services.grammar import (GRAMMAR, START_SYMBOL, EPSILON, EOF, is_non_terminal)
...
class GrammarAnalyzer:
        ...
        for productions in GRAMMAR.values():
            for production in productions:
                for symbol in production:
                    if (symbol != EPSILON and not is_non_terminal(symbol)):
    ...
    def first_symbol(cls, symbol):
        ...
        for production in GRAMMAR[symbol]:
            first_production = cls.first_sequence(production)
            first.update(first_production)
        return first
            ...
            for non_terminal, productions in GRAMMAR.items():
                ...
                for production in productions:
                    ...
                    for symbol in production:
                        ...
                        production_first.update(symbol_first - {EPSILON})
    ...
    @classmethod
    def first_sequence_cached(cls, symbols, first):
            ...
            for lhs, productions in GRAMMAR.items():
                for production in productions:
                    for index, symbol in enumerate(production):
                        ...
                        first_beta = (cls.first_sequence_cached(beta, first))
    ...
    def analyze(cls):
        ...
        return {
            "first": first,
            "follow": follow,
            "non_terminals": cls.get_non_terminals(),
    ...
    def display(cls):
        ...
        for non_terminal in sorted(first):
            print(f"FIRST({non_terminal}) = " f"{sorted(first[non_terminal])}")
        ...
        for non_terminal in sorted(follow):
            ...
            print(f"FOLLOW({non_terminal}) = " f"{sorted(follow[non_terminal])}")
        print()