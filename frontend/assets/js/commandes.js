function sendCommand() {
    ...
    .then(data => {
        ...
        if (data.execution && data.execution.show_form) {
            if ( data.execution.form_type === "add_question" ) {
                showAddQuestionForm();
            }
            else if (data.execution.form_type === "edit_question") {
...
function showAddQuestionForm() {
    document.getElementById("dynamicForm").innerHTML = `
        ...
        <button onclick="submitAddQuestion()">Enregistrer</button>
    `;
}
...
function showEditQuestionForm(questionId){
    document.getElementById("dynamicForm").innerHTML = `
        <h3>Modification de la Question ${questionId}</h3>
        <input type="text" id="texte_question" placeholder="Veuiller saisir la nouvelle question">
        <input type="number" id="ordre_question" placeholder="Entrer le numero d'ordre">
        <button onclick="submitEditQuestion(${questionId})">Modifier</button>
    `;
}
